...
/* ----------------------- Modbus includes ----------------------------------*/
//#include "modbus/include/mb.h"               //modify by alan (get comment)
//#include "modbus/include/mbport.h"
#include "mb.h"                               // modify by alan
#include "mbport.h"
...
void            vMBPortLog( eMBPortLogLevel eLevel, const CHAR * szModule,
                            const CHAR * szFmt, ... );
		char modbustcp_recvbuf[300];				//add by GAS
...
//			/**************Add by GAS**************/
//			/************************
//			ÓÃÓÚ²Ù×÷000¼Ä´æÆ÷ºÍ001¼Ä´æÆ÷
//			000=>LED1
//			001=>LED0
//			***********************/
//				memcpy( modbustcp_recvbuf, p->payload, p->len );
//				if(modbustcp_recvbuf[14]==0x01)HAL_GPIO_WritePin(GPIOB, DS1_Pin, GPIO_PIN_RESET);
//				else	if(modbustcp_recvbuf[14]==0x00)HAL_GPIO_WritePin(GPIOB, DS1_Pin, GPIO_PIN_SET);
//				if(modbustcp_recvbuf[16]==0x01)HAL_GPIO_WritePin(GPIOB, DS0_Pin, GPIO_PIN_RESET);
//				else	if(modbustcp_recvbuf[14]==0x00)HAL_GPIO_WritePin(GPIOB, DS0_Pin, GPIO_PIN_SET);
//      /*****************over*****************/  
			usTCPBufPos += p->len;
...
xMBTCPPortSendResponse( const UCHAR * pucMBTCPFrame, USHORT usTCPLength )
{
    ...
    if( pxPCBClient )
    {
        ...
        if( tcp_write( pxPCBClient, pucMBTCPFrame, ( u16_t ) usTCPLength,  0) == ERR_OK )	  //NETCONN_COPY
        {
					/*
#ifdef MB_TCP_DEBUG
