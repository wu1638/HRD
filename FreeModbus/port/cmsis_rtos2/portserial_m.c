...
#define huart                                   huart3
#define RS485_MASTER_TX_MODE()                  HAL_GPIO_WritePin(RS485_MASTER_CTRL_GPIO_Port, RS485_MASTER_CTRL_Pin)
#define RS485_MASTER_RX_MODE()                  HAL_GPIO_WritePin(RS485_MASTER_CTRL_GPIO_Port, RS485_MASTER_CTRL_Pin)
...
BOOL xMBMasterPortSerialPutByte(CHAR ucByte)
{
    //HAL_UART_Transmit_IT(&huart, (uint8_t*)&ucByte, 1);
		if(HAL_UART_Transmit (&huart ,(uint8_t *)&ucByte,1,0x01) != HAL_OK )	//添加发送一位代码
			return FALSE ;
		else
			return TRUE;
}
...
BOOL xMBMasterPortSerialPutBytes(volatile UCHAR *ucByte, USHORT usSize)
{
	...
	return TRUE;*/
	if(HAL_UART_Transmit (&huart ,(uint8_t *)&ucByte,usSize,0xFF) != HAL_OK )	//添加发送
			return FALSE ;
		else
			return TRUE;
}
